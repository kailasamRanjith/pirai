...
case 1:
   ...
   scanf("%d",&l[i].price);
  // printf("\nTitle=%s\tAuthor=%s\tYear=%d\tPrice=%d",l[i].title,l[i].author,l[i].year,l[i].price);
  // printf("i=%d",i);
    k++;
   // printf("k=%d",k);
...
case 2:
    ...
    case 3:
      ...
      //printf("Sort the details of application in incresed order of price:");
	for(i=0;i<k;i++)
	 {
	big[m]=l[i].price;
   ...
   case 5:
    ...
    /*printf("\nSort the details of application in incresed order author ");

   for(i=0;i<k;i++)
	 {
       strcpy(name[m],l[i].author);
       //	printf("m =%s",name[m]);
	m++;
	}
	  ...
	  for(n=m+1;n<k;n++)
	    {
	     if(strcmp(name[m],name[n])>0)
	      {
	      strcpy(temp[m],name[m]);
	      strcpy(name[m],name[n]);
	      strcpy(name[n],temp[m]);
	 }
	 }
	}
